...
main = do
  (d, haskell, nim, rust) <- mining
  BSL.writeFile "Bench/toPlot/d.csv" $ encode d
...
type Data = (Points, Points, Points, Points)
...
mining = do
  ...
  let proto1  = map (filter (/= "") . (splitOn "\n")) proto0
      ...
      d       = zip [1.0 ..] (map (!! 0) proto3)
      haskell = zip [1.0 ..] (map (!! 1) proto3)
      nim     = zip [1.0 ..] (map (!! 2) proto3)
      rust    = zip [1.0 ..] (map (!! 3) proto3)
  return (d, haskell, nim, rust)
