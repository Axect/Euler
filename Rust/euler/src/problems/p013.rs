...
pub fn ans() {
    let mut f = File::open("data/p013.csv").expect("There is no file here");
    let mut texts = String::new();
    f.read_to_string(&mut texts).expect(
    ...
    let result = texts
        ...
        .map(|x| {
            x.chars()
                .map(|t| t.to_digit(10u32).unwrap() as u64)
                .collect::<Row>()
        })
        .collect::<Matrix>();
...
impl LargeRow for Row {
    fn merge_sum(&self) -> String {
        let mut a = self.iter().rev().map(|x| *x as u64).collect::<Vec<u64>>();
        for i in 0..self.len() {
            let val = a[i];
            match &val {
                0..=9 => (),
